...
public class PrintLoginSuccessServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        * 깊은 복사를 통해 만들어서 전달하므로 데이터를 그대로 유지하고 있다. */
        ...
        /* forword 방식을 통해 다른 서블릿으로 위임하더라도 데이터를 공유 할 수 있다.
        * forword 동작을 클라이언트가 알 필요가 없으므로 url은 변경되지 않는다.
        * 새로고침(= 재요청)을 하게 되면 동일한 요청을 반복하게 되는데 이 때 데이터베이스에 insert 하는
        * 등의 행위를 하게 되면 중복 행 삽입 가능성이 있다. (조회 select는 o / insert, update, delete x)
